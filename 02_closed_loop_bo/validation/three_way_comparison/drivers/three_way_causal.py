#!/usr/bin/env python
"""Causal (per-cycle) GP holdout three-way — the GP AS IT ACTUALLY RAN in the workflow.
...
MAXV = 400.0   # reward = K0/MAXV  ->  K0(GPa) = reward*MAXV (the workflow GP's target scale)
RB = HCAP + "/results-paper-v4/results_bm"
ESEN = HCAP + "/analysis/top_structures/bm/global_top20.csv"
STRUCT = DATA_ROOT + "/structures"
DFT = K0DIR
OUT = OUT_ROOT + "/results/three_way_causal.csv"
OUTALL = OUT_ROOT + "/results/gp_causal_allpreds.csv"
...
def main():
    # ---- DFT-validated targets, keyed by exact cif text (match against memory) ----
    ...
    for cif in sorted(glob.glob(STRUCT + "/bm_*.cif")):
        ...
        if not os.path.exists(dftj) or rank > len(esen):
            continue
        txt = open(cif).read().strip()
        tgt[txt] = dict(stem=stem, rank=rank, formula=esen.iloc[rank - 1]["reduced_formula"],
                        dft_K0=json.load(open(dftj))["B0_GPa"], esen_K0=float(esen.iloc[rank - 1]["value"]))
    print("DFT-validated targets to match in the loop: %d" % len(tgt), flush=True)

    # ---- gather ALL unique generated structures across runs; freeze ORB->PCA50 once ----
    runs = sorted(glob.glob(RB + "/bm_p3v4_bm_*"))
    per_run = []
    allcifs = {}
    for run in runs:
