...
def _plot_single(df: pd.DataFrame, metric: str, dataset_key: str, out_name: str) -> Path:
    ...
    # PHEASY-SI: pheasy spans n_train up to 2000 (GP) / 1000 (MTGP, DGP) — label every n present.
    xt = sorted(int(v) for v in grouped["n_train"].unique())
    ax.set_xticks(xt)
...
def main() -> None:
    ...
    # RMSE learning curve dropped for the SI: averaging RMSE across Cv/S/F/omega_max (very
    # different magnitudes; F dominates) makes the mean+/-std uninterpretable. Keep R2 + Spearman.
    plan: list[tuple[str, str, str]] = [
        ("phonon", "R2", "fig_combined_pheasy_R2_learning_curve.png"),
        ("phonon", "Spearman", "fig_combined_pheasy_Spearman_learning_curve.png"),
    ]
