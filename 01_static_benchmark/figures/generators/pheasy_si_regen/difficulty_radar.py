...
FIG_OUT = Path(__file__).resolve().parent / "out_pheasy"
...
DATASETS = {
    ...
    "phonon": {
        ...
        "prop_label": {"Cv_300K": "C$_V$", "S_300K": "S", "F_300K": "F",
                       "max_phonon_freq": r"$\omega_{\mathrm{max}}$"},
        "diff_out": FIG_OUT / "fig_difficulty_pheasy_n500.pdf",
        "radar_out": FIG_OUT / "fig_radar_pheasy_orb_R2_n500.pdf",
        "label": "Phonon (Pheasy)",
    },
    "elastic": {
...
def main() -> None:
    apply_style()
    FIG_OUT.mkdir(parents=True, exist_ok=True)
    for key, cfg in [("phonon", DATASETS["phonon"])]:  # PHEASY-SI: only the big phonon dataset
