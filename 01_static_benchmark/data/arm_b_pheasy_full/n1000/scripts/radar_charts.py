...
def plot_radar_chart_orb(radar_data, metric='R2', output_dir='../figures/radar_charts'):
    ...
    for model in models:
        ...
        model_data = model_data.set_index('property').loc[properties].reset_index()
    ...
    title = f'Surrogate Comparison Across Properties - ORB Descriptor\n' \
            f'{metric} (n=1000, Best PCA per Surrogate per Property)'
    ...
    if print_pca_table:
        ...
        for prop in properties:
            row = [prop]
            for model in models:
                model_prop_data = radar_data[
                    (radar_data['model'] == model) &
                    (radar_data['property'] == prop)
                ]
                if not model_prop_data.empty:
                    pca = model_prop_data['best_pca'].values[0]
                    row.append(f"PCA={pca}")
                else:
                    row.append("N/A")
        ...
        header = f"{'Property':25s} " + " ".join(f"{lbl:15s}" for lbl in ['GP', 'MTGP', 'DGP'])
    ...
    # Save figure
    output_path = os.path.join(output_dir, f'orb_{metric}_n1000.png')
...
def main():
    ...
    data_path = os.path.join(script_dir, '..', 'data', 'filtered_n1000.csv')
