"""eSEN-30M-OAM heat-capacity runner via fairchem.OCPCalculator + phonopy.

Same I/O contract as run_orb_phonon.py and run_uma_phonon.py:
    python runners/run_esen.py <xyz_in> <out_txt> <n_workers>
    -> Cp@300K (J/g/K), one per line, NaN on failure.

Pipeline mirrors run_orb_phonon.py exactly; only the calculator changes
to fairchem-core's OCPCalculator with model_name='eSEN-30M-OAM'.

Note: bypasses quacc entirely — quacc 0.12 doesn't recognize method='fairchem',
and quacc >=1.0 expects the 2.x FAIRChemCalculator API. This direct phonopy
approach is consistent across all three oracle backends.
"""

from __future__ import annotations

import gc
import multiprocessing as mp
import os
import sys

os.environ.setdefault("QUACC_RESULTS_DIR", "/tmp")

import numpy as np
import torch
from ase.io import read
from ase.optimize import FIRE
try:
    from ase.filters import ExpCellFilter
except ImportError:
    from ase.constraints import ExpCellFilter
from pymatgen.io.ase import AseAtomsAdaptor


_ESEN_CALC = None  # singleton per process


def _load_esen():
    """Load fairchem OCPCalculator with eSEN-30M-OAM (singleton)."""
    global _ESEN_CALC
    if _ESEN_CALC is None:
        from fairchem.core import OCPCalculator
        local_cache = os.environ.get(
            "FAIRCHEM_LOCAL_CACHE",
            "/scratch/user/ahnafalvi/hf_cache",
        )
        _ESEN_CALC = OCPCalculator(
            model_name="eSEN-30M-OAM",
            local_cache=local_cache,
            cpu=not torch.cuda.is_available(),
        )
    return _ESEN_CALC


def _supercell_matrix_for_min_length(cell, min_len: float = 10.0) -> np.ndarray:
    a, b, c = np.linalg.norm(cell, axis=1)
    return np.diag([
        max(1, int(np.ceil(min_len / a))),
        max(1, int(np.ceil(min_len / b))),
        max(1, int(np.ceil(min_len / c))),
    ])


def _phonon_task(atoms):
    try:
        from phonopy import Phonopy
        from phonopy.structure.atoms import PhonopyAtoms
    except Exception as e:
        print(f"[esen] phonopy import failed: {e}", file=sys.stderr)
        return np.nan

    try:
        calc = _load_esen()

        # 1. Relax cell + positions with FIRE.
        a = atoms.copy()
        a.calc = calc
        try:
            opt = FIRE(ExpCellFilter(a), logfile=None)
            opt.run(fmax=0.01, steps=500)
        except Exception as e:
            print(f"[esen] relax failed: {e}", file=sys.stderr)
            return np.nan

        # 2. Build phonopy supercell with min_lengths >= 10 Å.
        sc_matrix = _supercell_matrix_for_min_length(a.get_cell().array, 10.0)
        unit = PhonopyAtoms(
            symbols=a.get_chemical_symbols(),
            scaled_positions=a.get_scaled_positions(),
            cell=a.get_cell().array,
        )
        phonon = Phonopy(unit, supercell_matrix=sc_matrix)
        phonon.generate_displacements(distance=0.01)
        sc_atoms = phonon.supercells_with_displacements

        # 3. Compute forces on each displaced supercell.
        forces = []
        for s in sc_atoms:
            from ase import Atoms as ASEAtoms
            asx = ASEAtoms(
                symbols=s.symbols,
                scaled_positions=s.scaled_positions,
                cell=s.cell,
                pbc=True,
            )
            asx.calc = calc
            forces.append(asx.get_forces())
        phonon.forces = forces
        phonon.produce_force_constants()

        # 4. Mesh + thermal properties at 0..300 K (10 K steps).
        phonon.run_mesh([20, 20, 20])
        phonon.run_thermal_properties(t_step=10, t_max=300, t_min=0)
        tp = phonon.get_thermal_properties_dict()

        # 5. Reject imaginary modes (any frequency below -0.1 THz).
        mesh = phonon.get_mesh_dict()
        if mesh is not None and (mesh["frequencies"] < -0.1).any():
            print("[esen] imaginary modes detected", file=sys.stderr)
            return np.nan

        # 6. Cp at T=300K → J/g/K.
        temps = np.asarray(tp["temperatures"])
        idx = int(np.argmin(np.abs(temps - 300.0)))
        cv_unit = float(tp["heat_capacity"][idx])  # J/K/mol of unit cell
        struc = AseAtomsAdaptor.get_structure(atoms)
        cp_per_g = cv_unit / float(struc.composition.weight)

        gc.collect()
        if torch.cuda.is_available():
            torch.cuda.empty_cache()
        return cp_per_g

    except Exception as e:
        print(f"[esen] failed: {type(e).__name__}: {e}", file=sys.stderr)
        return np.nan


def main():
    if len(sys.argv) != 4:
        print("usage: run_esen.py <xyz_in> <out_txt> <n_workers>", file=sys.stderr)
        sys.exit(2)
    xyz_in, out_txt, n_workers = sys.argv[1], sys.argv[2], int(sys.argv[3])
    atoms_list = read(xyz_in, index=":")
    if not isinstance(atoms_list, list):
        atoms_list = [atoms_list]

    if n_workers <= 1:
        results = [_phonon_task(a) for a in atoms_list]
    else:
        with mp.Pool(processes=n_workers) as pool:
            results = pool.map(_phonon_task, atoms_list)

    np.savetxt(out_txt, np.array(results, dtype=np.float64), fmt="%.6f")


if __name__ == "__main__":
    main()
