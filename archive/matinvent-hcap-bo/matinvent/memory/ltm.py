...
class LongTimeMem:
    ...
    def __init__(
        ...
        # NEW: Added columns for GP training
        self.memory = pd.DataFrame(
            columns=[
                "struc", "comp", "ele_comb", "reward", "RL_step",
                "property_value",    # NEW: Actual property value for GP training
                "features",          # NEW: ORB features (cached)
                "calculator_used",   # NEW: Which calculator was used
                "structure_id",      # NEW: ID for matching structures across calculators
            ]
        )
        self.unique_comps = []
        self._structure_id_counter = 0  # Auto-increment ID
    ...
    def extend(
        ...
        step: int,
        property_values: np.ndarray[float] = None,  # NEW: for GP training
        features: np.ndarray = None,                 # NEW: ORB features
        calculators_used: List[str] = None,          # NEW: calculator metadata
        structure_ids: List[int] = None              # NEW: IDs for matching structures
    ) -> None:
        ...
        # Build DataFrame with new optional columns
        df_data = {
            "struc": strucs,
            "comp": comps,
            "ele_comb": ele_comb,
        ...
        # Add NEW columns if provided (backwards compatible)
        if property_values is not None:
            df_data["property_value"] = property_values
        else:
            df_data["property_value"] = [None] * len(strucs)

        if features is not None:
            df_data["features"] = list(features)  # Convert array to list for storage
        else:
            df_data["features"] = [None] * len(strucs)

        if calculators_used is not None:
            df_data["calculator_used"] = calculators_used
        else:
            df_data["calculator_used"] = [None] * len(strucs)

        # Add structure IDs (for matching structures across calculators)
        if structure_ids is not None:
            df_data["structure_id"] = structure_ids
        else:
            # Auto-generate sequential IDs
            df_data["structure_id"] = list(range(
                self._structure_id_counter,
                self._structure_id_counter + len(strucs)
            ))
            self._structure_id_counter += len(strucs)

        df_sample = pd.DataFrame.from_dict(df_data)
