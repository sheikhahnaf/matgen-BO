...
def get_best_pca_per_surrogate_per_property(df, property_name, metric='R2'):
    ...
    Args:
        df: DataFrame with filtered n=2000 data
...
def plot_bar_chart_per_property(best_configs_df, property_name, metric='R2',
    ...
    for i, model in enumerate(models):
        ...
        # Skip if no valid data for this model (e.g., DGP for n=2000, n=2000)
    ...
    # Set title
    title = f'{property_name}: {metric}\n(n=2000, Best PCA per Surrogate for This Property)'
    ...
    # Save figure
    output_path = os.path.join(output_dir, f'{property_name}_{metric}_n2000.png')
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    print(f"    Saved: {property_name}_{metric}_n2000.png")
