"""
Phase 0: Data Preparation for Simplified ASE Regression Analysis (n=250 only)

This script:
1. Loads aggregated results from previous analysis
2. Filters to n=250 only
...
def load_and_filter_data():
    ...
    """
    # Load data from analysis_v3 directory
    ...
    # Filter to n=250
    df = df[df['n_train'] == 250].copy()
    print(f"\nAfter filtering to n=250: {df.shape}")
    ...
    expected_rows = 3 * 4 * 3 * 4 * 3  # models × descriptors × PCA × 4 properties × metrics
...
def save_filtered_data(df):
    ...
    output_path = os.path.join(output_dir, 'filtered_n250.csv')
