...
def plot_heatmap_averaged(best_configs_df, metric='R2', output_dir='../figures/heatmaps'):
    ...
    pivot = pivot.reindex(index=['orb', 'mace', 'uma', 'soap'], columns=['gp', 'mtgp_2', 'dgp'])
    ...
    # Uppercase labels
    ax.set_xticklabels(['GP', 'MTGP', 'DGP'], fontsize=12)
    ...
    for descriptor in ['orb', 'mace', 'uma', 'soap']:
        for model in ['gp', 'mtgp_2', 'dgp']:
            ...
            pca_annotations.append(f"{model_label}-{descriptor.upper()}:{pca_rows[0]}")
