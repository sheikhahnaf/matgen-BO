...
def get_best_pca_per_surrogate_per_property(df, property_name, metric='R2'):
    ...
    Args:
        df: DataFrame with filtered n=500 data
...
def plot_bar_chart_per_property(best_configs_df, property_name, metric='R2',
    ...
    for i, model in enumerate(models):
        ...
        model_data = model_data.set_index('descriptor').reindex(descriptors).reset_index()
    ...
    # Set title
    title = f'{property_name}: {metric}\n(n=500, Best PCA per Surrogate for This Property)'
    ...
    for descriptor in descriptors:
        ...
        pca_str = []
        for model in models:
            model_rows = desc_data[desc_data['model'] == model]['best_pca'].values
            if len(model_rows) > 0:
                pca_str.append(f"{model_labels[model]}:{model_rows[0]}")
    ...
    # Save figure
    output_path = os.path.join(output_dir, f'{property_name}_{metric}_n500.pdf')
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    print(f"    Saved: {property_name}_{metric}_n500.pdf")
