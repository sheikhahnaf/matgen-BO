        ...
        for descriptor in descriptors:
            for prop in properties:
                for pca in pcas:
                    ...
                    # Get R2 and Spearman
                    r2_rows = subset[subset['metric'] == 'R2']['mean'].values
                    sp_rows = subset[subset['metric'] == 'Spearman']['mean'].values
                    if len(r2_rows) == 0 or len(sp_rows) == 0:
                        continue
                    r2 = r2_rows[0]
                    spearman = sp_rows[0]
...
def plot_pca_sensitivity_averaged(sensitivity_df, metric='R2',
    ...
    # Save figure
    output_path = os.path.join(output_dir, f'averaged_{metric}_n500.pdf')
...
def plot_pca_sensitivity_per_property(sensitivity_df, metric='R2',
    ...
    plt.suptitle(f'PCA Sensitivity Per Property: {metric} (ORB Descriptor, n=500)',
                fontsize=16, fontweight='bold', y=0.995)

    plt.tight_layout()

    # Save figure
    output_path = os.path.join(output_dir, f'per_property_{metric}_n500.pdf')
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    print(f"  Saved: {output_path}")
