...
def run_descriptor(descriptor, df_train, df_holdout, target_cols, args, output_dir):
    ...
    # DATA-1 audit fix: holdout rows that could not be genuinely featurized
    # (SOAP unknown-species fallback, or MACE/ORB/UMA None->zeros) carry a
    # zero/constant feature vector. Excluding them from the per-property holdout
    # mask below keeps such rows out of HOLDOUT metrics (training is unchanged).
    ...
    for prop in target_cols:
        ...
        # DATA-1: drop holdout rows with invalid/unknown-species features from the
        # holdout metric set (target-not-NaN AND genuinely featurized). Training
        # is untouched.
        valid_holdout_target = valid_holdout
        valid_holdout = valid_holdout_target & holdout_feat_valid
        n_excluded = int(valid_holdout_target.sum() - valid_holdout.sum())
        print(f"[holdout][{descriptor}] excluded {n_excluded}/{int(valid_holdout_target.sum())} "
              f"rows (invalid/unknown-species features) for {prop}")
