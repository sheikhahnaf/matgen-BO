"""
Data Aggregation Script for Phonon-Thermo Benchmark -- Arm B (Pheasy reference)

This script loads all *_holdout_summary.csv files from the GP and MTGP_2 result
directories tagged with the 'pheasy' dataset prefix and creates a unified
...
convention results/<model>/pheasy_pca<P>_n<N>/<model>_<descriptor>_holdout_summary.csv.

Note: Arm B does NOT include DGP, and the n_train range is extended
(GP up to 2000, MTGP up to 1000) -- see N_TRAIN_BY_MODEL.
"""
...
# Arm B dataset tag. Result directories are named e.g. 'pheasy_pca10_n1000'.
DATASET_PREFIX = 'pheasy'

# Arm B surrogate models (NO DGP on Arm B).
MODELS = ['gp', 'mtgp_2']

# Coverage grid for the missing-combination report (Arm B).
# These only affect the coverage report, NOT the aggregated metrics.
DESCRIPTORS = ['soap', 'mace', 'orb', 'uma']
PCA_VALUES = [10, 25, 50]
# n_train grid is per-model (kept tolerant): GP extends to 2000, MTGP to 1000.
# A model not listed here falls back to N_TRAIN_DEFAULT.
N_TRAIN_BY_MODEL = {
    'gp': [100, 250, 500, 1000, 2000],
    'mtgp_2': [100, 250, 500, 1000],
}
N_TRAIN_DEFAULT = [100, 250, 500, 1000]
...
def parse_config_from_path(csv_path: Path) -> Dict[str, str]:
    ...
    Example paths:
        results/gp/pheasy_pca10_n100/gp_soap_holdout_summary.csv
        results/mtgp_2/pheasy_pca25_n1000/mtgp_mace_holdout_summary.csv
    ...
    dir_name = csv_path.parent.name  # e.g., 'pheasy_pca10_n100'
...
def load_all_results(results_dir: Path, models: List[str] = None) -> pd.DataFrame:
    ...
    for model in models:
        ...
        # prefix (e.g. 'pheasy_pca10_n1000'). This excludes smoke-test CSVs that
    ...
    if not all_dfs:
        # Arm B may legitimately have zero results during early/partial runs.
        # Return an empty, correctly-typed frame so downstream code can proceed
        # without a hard crash on the coverage step.
        print("WARNING: No pheasy CSV files found yet -- returning empty DataFrame.")
        empty = pd.DataFrame(columns=[
            'model', 'descriptor', 'pca_components', 'n_train',
            'property', 'metric', 'mean', 'std', 'config', 'cv', 'relative_std'
        ])
        return empty
...
def identify_missing_combinations(df: pd.DataFrame) -> pd.DataFrame:
    ...
    The expected grid is per-model (N_TRAIN_BY_MODEL) so that Arm B's
    model-specific n_train ranges report coverage correctly (GP to 2000, MTGP
    to 1000, no DGP). Only models actually present in the data are checked, so a
    partial run does not flag every model as missing.
    """
    expected_combinations = []

    models = df['model'].unique() if len(df) else []
    ...
    expected_df = pd.DataFrame(
        expected_combinations,
        columns=['model', 'descriptor', 'pca_components', 'n_train']
    )

    if len(df) == 0:
        # Nothing aggregated yet -> nothing to compare against.
        return expected_df.iloc[0:0]
...
def generate_summary_report(df: pd.DataFrame, output_path: Path):
    ...
    if len(df) == 0:
        lines.append("No results aggregated yet for this arm (0 rows).")
        lines.append("")
        lines.append("=" * 80)
        report = "\n".join(lines)
    ...
    if len(missing) > 0:
        ...
        for _, row in missing.iterrows():
            lines.append(f"  - {row['model']:10s} {row['descriptor']:4s} "
                        f"PCA={row['pca_components']:2d} n={row['n_train']:4d}")
    else:
...
def main():
    ...
    print(f"\nLoaded {len(df)} records from "
          f"{df['config'].nunique() if len(df) else 0} configurations")
