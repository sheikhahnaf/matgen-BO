...
def plot_radar_chart_orb(radar_data, metric='R2', output_dir='../figures/radar_charts'):
    ...
    title = f'Surrogate Comparison Across Properties - ORB Descriptor\n' \
            f'{metric} (n=2000, Best PCA per Surrogate per Property)'
    ...
    if print_pca_table:
        ...
        header = f"{'Property':25s} " + " ".join(f"{lbl:15s}" for lbl in ['GP'])
    ...
    # Save figure
    output_path = os.path.join(output_dir, f'orb_{metric}_n2000.png')
...
def main():
    ...
    data_path = os.path.join(script_dir, '..', 'data', 'filtered_n2000.csv')
