...
def plot_pca_sensitivity_averaged(sensitivity_df, metric='R2',
    ...
    # Save figure
    output_path = os.path.join(output_dir, f'averaged_{metric}_n2000.png')
...
def plot_pca_sensitivity_per_property(sensitivity_df, metric='R2',
    ...
    plt.suptitle(f'PCA Sensitivity Per Property: {metric} (ORB Descriptor, n=2000)',
                fontsize=16, fontweight='bold', y=0.995)

    plt.tight_layout()

    # Save figure
    output_path = os.path.join(output_dir, f'per_property_{metric}_n2000.png')
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    print(f"  Saved: {output_path}")
