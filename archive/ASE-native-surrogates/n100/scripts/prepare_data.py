"""
Phase 0: Data Preparation for Simplified ASE Regression Analysis (n=100 only)

This script:
1. Loads aggregated results from previous analysis
2. Filters to n=100 only
...
def load_and_filter_data():
    ...
    input_path = os.path.join(script_dir, '..', '..', 'ASE_regression_test', 'analysis', 'aggregated_results.csv')
    ...
    # Filter to n=100
    df = df[df['n_train'] == 100].copy()
    print(f"\nAfter filtering to n=100: {df.shape}")
    ...
    expected_rows = 3 * 4 * 3 * 8 * 3  # models × descriptors × PCA × properties × metrics
...
def save_filtered_data(df):
    ...
    output_path = os.path.join(output_dir, 'filtered_n100.csv')
